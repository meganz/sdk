...
TEST(PayCrypterTest, allFeatures)
{
    ...
    //Check result
    char base64Result[result.size()*4/3+4];
    ...
    string rsaRes;
    payCrypter.rsaEncryptKeys(&message, SDK_PUBKEY, &rsaRes, false);

    //Check result
    char rsaResult[rsaRes.size()*4/3+4];
    ...
    payCrypter.rsaEncryptKeys(&message, SDK_PUBKEY, &cryptKeysBytesBin, false);

    //Check result
    char cryptKeysBytes[cryptKeysBytesBin.size()*4/3+4];
