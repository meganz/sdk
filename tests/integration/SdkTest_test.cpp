...
TEST_F(SdkTest, SdkTestMegaVpnCredentials)
{
    ...
    // Get VPN regions to choose one of them
    {
        ...
        if (isProAccount)
        {
            ...
            ASSERT_TRUE(userPubKey.size() == expectedPubKeyB64Size) << "User Public Key does not have the expected size. Size: " << userPubKey.size() << ". Expected: " << expectedPubKeyB64Size;
            ASSERT_FALSE(userPubKey.empty()) << "VPN Credential data is EMPTY";
        }
        else
        {
        ...
        // Delete VPN credentials on the used slot
        {
            ...
            else
            {
                ...
                ASSERT_EQ(API_ENOENT, result) << "deleting the VPN credentials on a never occupied slotID " << slotID << " didn't return the expected error value (error: " << result << ")";
            }
        }
