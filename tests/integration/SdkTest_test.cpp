...
TEST_F(SdkTest, SdkTestMegaVpnCredentials)
{
    ...
    // Get VPN regions to choose one of them
    {
        ...
        if (isProAccount)
        {
            ASSERT_EQ(API_OK, result) << "deleting the VPN credentials on the slotID " << slotID << " failed";

            // 6) Delete VPN credentials again on the same slotID -expected API_ENOENT: SlotID is empty-
            result = doDelVpnCredential(0, slotID);
        }
        ASSERT_EQ(API_ENOENT, result) << "deleting the VPN credentials on the unused slotID " << slotID << " failed";

        // 7) Delete VPN credentials with an invalid slot -expected EARGS: SlotID is not valid-
        result = doDelVpnCredential(0, -1);
        ASSERT_EQ(API_EARGS, result) << "deleting the VPN credentials failed for invalid slotID " << slotID;
    }
}
