...
// TRANSFER METRICS TEST

// Helper function to create Metrics with shared logic.
stats::TransferStats::Metrics calculateExpectedMetrics(direction_t transferType,
                                                       const std::vector<m_off_t>& sizes,
                                                       double raidedTransferRatio = 0.0)
{
    stats::TransferStats::Metrics metrics;
    metrics.transferType = transferType;
    ...
    if (n % 2 == 0)
    {
        metrics.medianSize = static_cast<m_off_t>(
            std::round(static_cast<double>(sortedSizes[n / 2 - 1] + sortedSizes[n / 2]) / 2.0));
    }
    else
    {
        metrics.medianSize = sortedSizes[n / 2];
    }

    // Calculate weighted average size (weighted by the size itself).
    m_off_t sizeSum = std::accumulate(sizes.begin(), sizes.end(), 0LL);
    m_off_t weightedSizeSum = std::accumulate(sizes.begin(),
                                              sizes.end(),
                                              0LL,
                                              [](m_off_t sum, m_off_t size)
                                              {
                                                  return sum + size * size;
                                              });
    metrics.weightedAverageSize = static_cast<m_off_t>(
        std::round(static_cast<double>(weightedSizeSum) / static_cast<double>(sizeSum)));

    // Set RAID transfer ratio.
    metrics.raidedTransferRatio = raidedTransferRatio;
 ...
 *    2.1 Collect metrics.
 *    2.2 Declare helper lambda function to compare metrics.
 *    2.3 Define sizes of uploaded and regular downloaded files.
 *    2.4 Define expected metrics for uploads and compare results.
 *    2.5 Define expected metrics for the regular downloads and compare results.
 ...
 */
TEST_F(SdkTest, SdkTestTransferStats)
{
    ...
    // 1.1 Upload files.
    auto uploadFileForStats = [this, &rootNode](const std::string_view uploadFileName,
                                                const std::string_view content) -> MegaNode*
    {
        MegaHandle fileHandle = 0;
    ...
    std::unique_ptr<MegaNode> testFileNode1(uploadFileForStats("test1.txt", file1content));
    ASSERT_TRUE(testFileNode1);

    constexpr std::string_view file2content = "Current content 2";
    std::unique_ptr<MegaNode> testFileNode2(uploadFileForStats("test2.txt", file2content));
    ASSERT_TRUE(testFileNode2);

    // 1.2. Download both files.
    auto downloadFileForStats = [this](MegaNode* node, const std::string_view downloadFileName)
    {
        ASSERT_EQ(
            MegaError::API_OK,
            doStartDownload(0,
    ...
    const stats::TransferStats::Metrics uploadMetrics =
        client->transferStatsManager.collectAndPrintMetrics(PUT);
    ...
    const stats::TransferStats::Metrics downloadMetrics1 =
        client->transferStatsManager.collectAndPrintMetrics(GET);

    // 2.2 Declare helper lambda function to compare metrics.
    auto compareMetrics = [](const stats::TransferStats::Metrics& expected,
                             const stats::TransferStats::Metrics& actual)
    {
        ...
        EXPECT_EQ(expected.transferType, actual.transferType);
        EXPECT_EQ(expected.medianSize, actual.medianSize);
        EXPECT_EQ(expected.weightedAverageSize, actual.weightedAverageSize);
        EXPECT_TRUE(actual.medianSpeed > 0);
        EXPECT_TRUE(actual.weightedAverageSpeed >= actual.medianSpeed);
        EXPECT_TRUE(actual.maxSpeed >= actual.medianSpeed);
        EXPECT_TRUE(actual.avgLatency > 0 && actual.avgLatency < 1000);
        EXPECT_TRUE(actual.failedRequestRatio >= 0.0 && actual.failedRequestRatio <= 1.1);
        EXPECT_EQ(expected.raidedTransferRatio, actual.raidedTransferRatio);
    };
