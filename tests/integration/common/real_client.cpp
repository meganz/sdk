...
class RealClient::RealContact: public Contact
{
...
class RealClient::RealInvite: public Invite
{
...
Error RealClient::fetch(bool ignoreCache)
{
    ...
    // Ask client to describe our cloud content.
    {
        ...
        auto result = mPendingRequests.emplace(std::move(key), std::move(completion));
                        ...
                        const std::string& salt)
{
    ...
    do
    {
        ...
        if (accountVersion == 1)
        {
            ...
            mClient->login(email.c_str(), passwordKey, nullptr, std::move(completion));
        ...
        // Try and log the user in.
        mClient->login2(email.c_str(), password.c_str(), &salt, nullptr, std::move(completion));
...
ErrorOr<std::string> RealClient::prelogin(const std::string& email)
{
    ...
    // Ask the client to perform prelogin.
    {
        ...
        mClient->prelogin(
            email.c_str(),
            std::bind(std::move(completion), std::placeholders::_4, std::placeholders::_3));
...
auto RealClient::invite(const std::string& email) -> ErrorOr<InvitePtr>
{
    ...
    // Send the invite.
    mClient->setpcr(email.c_str(), OPCA_ADD, nullptr, nullptr, UNDEF, std::move(invited));
...
Error RealClient::login(const std::string& email, const std::string& password)
{
...
Error RealClient::share(const std::string& email, CloudPath path, accesslevel_t permissions)
{
    // Sanity.
    assert(!email.empty());
...
bool RealClient::shared(const std::string& email, CloudPath path, accesslevel_t permissions) const
{
    // Sanity.
    assert(!email.empty());
...
Error RealClient::RealInvite::execute(opcactions_t)
{
    ...
    mClient.mClient->setpcr(nullptr, OPCA_DELETE, nullptr, nullptr, mID, std::move(executed));
...
Error RealClient::RealContact::remove()
{
    ...
    auto result = mClient.mClient->removecontact(mEmail.c_str(), HIDDEN, std::move(removed));
