...
class RealClient: public virtual Client, protected MegaApp
{
    class RealContact;
    class RealInvite;
    ...
    auto invited(const std::string& email, std::unique_lock<std::mutex>& lock) const -> InvitePtr;

    // Try and login the specified user.
    Error login(const std::string& email, const std::string& password, const std::string& salt);
