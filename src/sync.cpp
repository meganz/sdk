...
: localroot(new LocalNode)
{
    ...
    localroot->setScanAgain(true, true);
    localroot->setSyncAgain(true, true);
        ...
        if (LocalNode* sourceLocalNode = client->findLocalNodeByFsid(row.fsNode->fsid, row.fsNode->type, row.fsNode->fingerprint, *this, nullptr))
        {
            ...
            else
            {
                ...
                if (sourceCloudNode && !sourceCloudNode->mPendingChanges.empty())
                {
                    ...
                    client->mSyncFlags.ongoingMovesRenames = true;
                    rowResult = false;
                    return true;
                }
                ...
                if (sourceCloudNode && targetCloudNode)
                {
                    ...
                    if (sourceCloudNode->parent == targetCloudNode && newName.empty())
                    {
                        ...
                        parentRow.syncNode->setScanAgain(true, false);
                        if (sourceLocalNode->parent) sourceLocalNode->parent->setScanAgain(true, false);
                ...
                else
                {
                    ...
                    SYNC_verbose << client->clientname << "Source/Target unavaliable for move" << logTriplet(row, fullPath);
...
bool Sync::checkCloudPathForMovesRenames(syncRow& row, syncRow& parentRow, LocalPath& fullPath, bool& rowResult)
{
    ...
    else if (LocalNode* sourceLocalNode = client->findLocalNodeByNodeHandle(row.cloudNode->nodeHandle()))
    {
        ...
        else
        {
            ...
            SYNC_verbose << "Renaming/moving from the previous location: " << sourcePath.toPath(*client->fsaccess) << logTriplet(row, fullPath);
            sourceLocalNode->moveApplyingToCloud = true;
            rowResult = false;
            return true;
        }
...
dstime Sync::procextraq()
{
    ...
    while (queue.popFront(notification))
    {
        ...
        nearest->setScanAgain(true, !remainder.empty());
...
{
    assert(row.syncNode);
    SYNC_verbose << client->clientname << "Entering folder with syncagain=" << row.syncNode->syncAgain << " scanagain=" << row.syncNode->scanAgain << " at " << fullPath.toPath(*client->fsaccess);
        ...
        for (auto& childRow : childRows)
        {
            ...
            else
            {
                ...
                if (recurseHere &&
                    ...
                    !childRow.syncNode->moveApplyingToCloud &&
                    !childRow.syncNode->deletingFS &&   // we may not have visited syncItem if !syncHere, but skip these if we determine they are being moved from or deleted already
            ...
            if (row.cloudNode)
            {
                ...
                if (fsCloudEqual)
                {
                    ...
                    assert((!row.syncNode->slocalname && !row.fsNode->shortname) ||
                           (row.syncNode->slocalname && row.fsNode->shortname &&
                           *row.syncNode->slocalname == *row.fsNode->shortname &&
                           *row.syncNode->slocalname != row.syncNode->localname) ||
                           (!row.syncNode->slocalname && row.fsNode->shortname &&
                            *row.fsNode->shortname == row.syncNode->localname));
...
{
    if (client->mSyncFlags.scanningWasComplete ||
        //client->mSyncFlags.scansAndMovesComplete ||
    ...
    if (row.cloudNode->type == FILENODE)
    {
        ...
        if (parentRow.fsNode)
        {
            if (!row.syncNode->download)
            {
                ...
                LOG_debug << client->clientname << "Start sync download: " << logTriplet(row, fullPath);
    ...
    else
    {
        ...
        if (parentRow.fsNode)
        {
            ...
            if (client->fsaccess->mkdirlocal(fullPath))
            {
                ...
                //row.syncNode->localname = fullPath.leafName(client->fsaccess->localseparator);
                //if (auto sn = client->fsaccess->fsShortname(fullPath))
                //{
                //    if (*sn != row.syncNode->localname)
                //    {
                //        row.syncNode->slocalname = client->fsaccess->fsShortname(fullPath);
                //    }
                //}
            }
            else if (client->fsaccess->transient_error)
...
bool Sync::resolve_cloudNodeGone(syncRow& row, syncRow& parentRow, LocalPath& fullPath)
{
    ...
    else if (row.syncNode->deletingFS)
    {
        SYNC_verbose << client->clientname << "FS item already removed: " << logTriplet(row, fullPath);
        row.suppressRecursion = true;
    }
    else if (client->mSyncFlags.scanningWasComplete)//if (client->mSyncFlags.scansAndMovesComplete)
    {
...
bool Sync::resolve_fsNodeGone(syncRow& row, syncRow& parentRow, LocalPath& fullPath)
{
    ...
    if (!inProgress)
    {
        ...
        else if (client->mSyncFlags.scanningWasComplete)
        {
        ...
        else
        {
            ...
            SYNC_verbose << client->clientname << "Wait for scanning to finish before moving to local debris: " << logTriplet(row, fullPath);
        }
    }
