...
private:
    ...
    void logArchiveTimestamp_rotateFiles(const LocalPath& logsPath, const LocalPath& fileName)
    {
        ...
        {
            std::string leafName = leafNamePath.toPath(false);
    ...
    void logThreadFunction(LocalPath logsPath, LocalPath fileName)
    {
        ...
        while (!mLogExit)
        {
            ...
            if (mForceRenew)
            {
                ...
                {
                    threadErrors += "Error removing log file " + fileNameFullPath.toPath(false) + "\n";
                }
            ...
            else if (outFileSize > MAX_FILESIZE_MB*1024*1024)
            {
                ...
                {
                    threadErrors += "Failed to unlink log file: " + newNameZipping.toPath(false) + "\n";
                }
                if (!mFsAccess->renamelocal(fileNameFullPath, newNameZipping, true))
                {
                    threadErrors += "Failed to rename log file: " + fileNameFullPath.toPath(false) + " to " + newNameZipping.toPath(false) + "\n";
                }
