    ...
    if (transfer->localfilename.empty())
    {
        ...
        if (!sync->tmpfa)
        {
            ...
            while (i--)
            {
                ...
                transfer->localfilename.appendWithSeparator(tmpname, true, LocalPath::localPathSeparator);
                ...
                transfer->localfilename.appendWithSeparator(lockname, true, LocalPath::localPathSeparator);
        ...
        transfer->localfilename.appendWithSeparator(tmpfilename, true, LocalPath::localPathSeparator);
    }
...
void SyncFileGet::updatelocalname()
{
    ...
    if ((ait = n->attrs.map.find('n')) != n->attrs.map.end())
    {
        if (n->parent && n->parent->localnode)
        {
            ...
            localname.appendWithSeparator(LocalPath::fromName(ait->second, *sync->client->fsaccess, sync->mFilesystemType), true, LocalPath::localPathSeparator);
        }
    }
}
