...
TEST_P(FUSEPlatformTests, DISABLED_fstat_succeeds_after_directory_removed)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return access(MountPathO() / "sd0" / "sd0d0", F_OK) < 0 && errno == ENOENT &&
                   access(MountPathW() / "sd0" / "sd0d0", F_OK) < 0 && errno == ENOENT;
        },
        mDefaultTimeout));
...
TEST_P(FUSEPlatformTests, DISABLED_fstat_succeeds_after_file_removed)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return access(MountPathO() / "sf0", F_OK) < 0 && errno == ENOENT &&
                   access(MountPathW() / "sf0", F_OK) < 0 && errno == ENOENT;
        },
        mDefaultTimeout));
...
TEST_P(FUSEPlatformTests, futimes_success)
{
    ...
    for (auto i = 0; i < 2; ++i)
    {
        struct timeval times[] = {{i, i}, {i, i}}; // times
        ...
        ASSERT_TRUE(waitFor(
            [&]()
            {
                return !stat(MountPathO() / "sf0", buffer) && buffer.st_mtime == i;
            },
            mDefaultTimeout));
...
TEST_P(FUSEPlatformTests, move_local_file_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return access(MountPathO() / "sfx", F_OK) && errno == ENOENT &&
                   !stat(MountPathO() / "sfy", sfy) && sfxo == sfy;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, DISABLED_readdir_succeeds_when_changing)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return access(MountPathW() / "sd0", F_OK) && errno == ENOENT &&
...
TEST_P(FUSEPlatformTests, rename_move_rename_succeeds)
{
    ...
    ASSERT_FALSE(rename(MountPathW() / "sd0", MountPathW() / "sd1" / "sdx"));
...
TEST_P(FUSEPlatformTests, rename_move_succeeds)
{
    ...
    ASSERT_FALSE(rename(MountPathW() / "sd0", MountPathW() / "sd1" / "sd0"));
...
TEST_P(FUSEPlatformTests, rename_rename_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return !ClientW()->get("/x/s/sf0") && !access(MountPathO() / "sfx", F_OK);
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, rename_replace_directory_succeeds)
{
    ...
    ASSERT_FALSE(rename(MountPathW() / "sd0", MountPathW() / "sd1" / "sd1d0"));
...
TEST_P(FUSEPlatformTests, DISABLED_rename_replace_file_cloud_local_succeeds)
{
    ...
    EXPECT_TRUE(waitFor(
        [&]()
        {
            ...
            return access(MountPathO() / "sf0", F_OK) && errno == ENOENT &&
                   !stat(MountPathO() / "sfx", sfx) && sf0o == sfx;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, rename_replace_file_local_cloud_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        ...
        {
            return !ClientW()->get("/x/s/sf0") && access(MountPathO() / "sfx", F_OK) &&
                   errno == ENOENT && !stat(MountPathO() / "sf0", sf0) && sfxo == sf0;
        },
        mDefaultTimeout));
...
TEST_P(FUSEPlatformTests, rename_replace_file_local_local_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return !access(MountPathO() / "sfx", F_OK) && !access(MountPathO() / "sfy", F_OK);
        },
        mDefaultTimeout));
...
TEST_P(FUSEPlatformTests, DISABLED_rename_replace_file_succeeds)
{
    ...
    ASSERT_FALSE(rename(MountPathW() / "sf0", MountPathW() / "sd0" / "sd0f0"));
    ...
    EXPECT_TRUE(waitFor(
        [&]()
        {
            ...
            return !stat(MountPathO() / "sd0" / "sd0f0", after) && after == beforeO;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, rmdir_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            ...
            return access(MountPathW() / "sd0" / "sd0d0", F_OK) < 0 && errno == ENOENT;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, truncate_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            ...
            return !stat(MountPathO() / "sf0", buffer) && !buffer.st_size &&
                   !stat(MountPathO() / "sf1", buffer) && buffer.st_size == 32;
        },
        mDefaultTimeout));
...
TEST_P(FUSEPlatformTests, unlink_at_directory_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            ...
            return accessat(sd0, "sd0d0", F_OK) < 0 && errno == ENOENT;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, unlink_at_file_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            ...
            // File should no longer be visible by observer.
            return accessat(s, "sf0", F_OK) < 0 && errno == ENOENT;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, unlink_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            ...
            // File should no longer be visible by observer.
            return access(MountPathW() / "sf0", F_OK) < 0 && errno == ENOENT;
        },
        mDefaultTimeout));
}
...
TEST_P(FUSEPlatformTests, write_succeeds)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            return sfxO.read(&read[0], read.size(), 0) == read.size() && read == written;
        },
        mDefaultTimeout));
