...
struct FUSECommonTests: TestBase
{}; // FUSECommonTests
...
TEST_F(FUSECommonTests, file_cache_load)
{
    ...
    observer->expect({mount.name(), MOUNT_SUCCESS, MOUNT_DISABLED});
...
TEST_F(FUSECommonTests, share_changes_permissions)
{
    // Convenience.
    constexpr auto U_R =
        UNIX_OR_WINDOWS(fs::perms::owner_read, fs::perms::_File_attribute_readonly);
...
TEST_F(FUSECommonTests, supports_entities_with_international_names)
{
    ...
    ASSERT_TRUE(waitFor(
        [&]()
        {
            ...
            return fs::is_directory(dp, error) && !error && fs::is_regular_file(fp, error) &&
                   !error;
        },
        mDefaultTimeout));
...
handle fsidOf(const Path& path)
{
    ...
    return fsAccess.fsidOf(path.localPath(), false, true, FSLogging::logOnError);
}
