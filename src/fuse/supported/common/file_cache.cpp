...
void FileCache::remove(const FileIOContext& context, FileCacheLock lock)
{
...
FileIOContextRef FileCache::context(FileInodeRef file, bool inMemoryOnly) const
{
    assert(file);
...
FileInfoRef FileCache::info(const FileExtension& extension, InodeID id, bool inMemoryOnly) const
{
    FileCacheLock guard(*this);
    ...
    auto info =
        std::make_unique<FileInfo>(extension, *fileAccess, const_cast<FileCache&>(*this), id);
...
void ensureCachePathExists(Client& client, const LocalPath& path)
{
    ...
    throw FUSEErrorF("Unable to create file cache directory: %s", path.toPath(false).c_str());
}
