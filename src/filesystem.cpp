...
string LocalPath::leafOrParentName() const
{
    ...
    name = name.endsInSeparator() ? name.subpathTo(name.localpath.size() - 1).leafName() : name.leafName();

#ifdef WIN32
    if (!name.empty() && name.localpath.back() == L':')
    {
        name.localpath.pop_back(); // drop trailing ':'
    }
#endif
