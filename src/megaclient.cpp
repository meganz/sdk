...
void MegaClient::upgradeSecurity(std::function<void(Error)> completion)
{
    ...
    shares = mNodeManager.searchNodes(outsharesNf, CancelToken());
    for (auto &n : shares)
    {
        ++totalOutShares;
        if (n->sharekey)
    ...
    shares = mNodeManager.searchNodes(pendingOutSharesNf, CancelToken());
    for (auto &n : shares)
    {
        ++totalOutShares;
        if (n->sharekey)
    ...
    shares = mNodeManager.searchNodes(publicLinkNf, CancelToken());
    for (auto &n : shares)
    {
        ++totalOutShares;
        if (n->sharekey)
