...
error MegaClient::checkSyncConfig(SyncConfig& syncConfig, LocalPath& rootpath, std::unique_ptr<FileAccess>& openedLocalFolder, string& rootNodeName, bool& inshare, bool& isnetwork)
{
    ...
    if (syncs.backupRestrictionsEnabled() && syncConfig.isBackup() && remotenode->firstancestor()->nodeHandle() != mNodeManager.getRootNodeVault())
    {
...
void MegaClient::cleanupFailedBackup(const string& remotePath)
{
    ...
    if (getrootnode(n)->nodeHandle() != mNodeManager.getRootNodeVault())
    {
        LOG_err << "Failed to cleanup remote dir of backup "
                << remotePath
                << " (path outside Vault).";
        assert(getrootnode(n)->nodeHandle() == mNodeManager.getRootNodeVault());
        return;
    }

    node_list children = mNodeManager.getChildren(n);
    if (!children.empty())
    {
        LOG_err << "Failed to cleanup remote dir of backup "
                << remotePath
...
{
     // NOTE: In case of modidy/add some of this extension, the column mimetype from table nodes should be re-build
