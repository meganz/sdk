...
// export node link
error MegaClient::exportnode(Node* n, int del, m_time_t ets, bool writable, int tag, std::function<void(Error, handle, handle)> completion)
{
    ...
    {
    case FILENODE:
        getpubliclink(n, del, ets, writable, tag, move(completion));
        break;
    ...
    case FOLDERNODE:
        if (del)
        {
            ...
            getpubliclink(n, del, ets, writable, tag, nullptr);
        ...
        else
        {
            ...
            setshare(n, NULL, writable ? FULL : RDONLY, writable, nullptr, tag, [this, h, ets, tag, writable, completion](Error e, bool){
                ...
                {
                    getpubliclink(node, false, ets, writable, tag, completion);
                }
                else
                {
...
void MegaClient::getpubliclink(Node* n, int del, m_time_t ets, bool writable, int tag, std::function<void(Error, handle, handle)> f)
{
...
void MegaClient::fetchnodes(bool nocache)
{
    ...
    if ((loggedin() == FULLACCOUNT || loggedIntoFolder() ) &&
        !nodes.size() && sctable.get() && !ISUNDEF(cachedscsn) &&
        fetchsc(sctable.get()) && fetchStatusTable(statusTable.get()))
    {
        WAIT_CLASS::bumpds();
...
error MegaClient::isnodesyncable(Node *remotenode, bool *isinshare, SyncError *syncError)
{
    ...
    do {
        ...
        syncs.forEachRunningSync([&](Sync* sync) {
            ...
            if ((sync->active() || sync->paused()) && n == sync->cloudRoot())
            {
                anyAbove = true;
            }
        });
...
error MegaClient::pauseSync(Sync& sync)
{
    ...
    sync.changestate(SYNC_DISABLED, NO_SYNC_ERROR, false, true);
...
error MegaClient::resumeSync(Sync& sync)
{
    ...
    sync.changestate(SYNC_INITIALSCAN, NO_SYNC_ERROR, true, true);
