...
#endif
    , mAsyncQueue(*w, workerThreadCount), mFingerprints(*this)
{
    sctable = NULL;
...
void MegaClient::fetchnodes(bool nocache)
{
    ...
    if (loggedin() == FULLACCOUNT && !mNodes.size() && sctable && !ISUNDEF(cachedscsn) && fetchsc(sctable) && fetchStatusTable(statusTable))
    {
        WAIT_CLASS::bumpds();
...
node_list MegaClient::getChildren(Node* node)
{
    node_list nodeList;
    if (!node || !sctable)
    {
