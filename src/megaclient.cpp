...
error MegaClient::isLocalPathSyncable(string newPath, int newSyncTag, SyncError *syncError)
{
    ...
    for (const auto& config : syncConfigs->all())
    {
        ...
        if (config.getEnabled() && !isAnError(config.getError()) &&
                ( newLocallyEncodedAbsolutePath.isContainingPathOf(otherLocallyEncodedAbsolutePath, LocalPath::localPathSeparator)
                  || otherLocallyEncodedAbsolutePath.isContainingPathOf(newLocallyEncodedAbsolutePath, LocalPath::localPathSeparator)
                ) )
        {
...
error MegaClient::addsync(SyncConfig syncConfig, const char* debris, LocalPath* localdebris, SyncError &syncError, bool delayInitialScan, void *appData)
{
    ...
    rootpath.trimNonDriveTrailingSeparator(LocalPath::localPathSeparator);
...
bool MegaClient::syncdown(LocalNode* l, LocalPath& localpath, bool rubbish)
{
    ...
    for (node_list::iterator it = l->node->children.begin(); it != l->node->children.end(); it++)
    {
         ...
         && (l->parent || l->sync->debris != ait->second))
        {
            ...
            localpath.appendWithSeparator(LocalPath::fromName(ait->second, *fsaccess, l->sync->mFilesystemType), true, LocalPath::localPathSeparator);
    ...
    for (localnode_map::iterator lit = l->children.begin(); lit != l->children.end(); )
    {
        ...
        ScopedLengthRestore restoreLen(localpath);
        localpath.appendWithSeparator(ll->localname, true, LocalPath::localPathSeparator);
    ...
    for (rit = nchildren.begin(); rit != nchildren.end(); rit++)
    {
        ...
        ScopedLengthRestore restoreLen(localpath);
        localpath.appendWithSeparator(LocalPath::fromName(localname, *fsaccess, l->sync->mFilesystemType), true, LocalPath::localPathSeparator);
