...
std::string MegaClient::getAuthURI(bool supressSID, bool supressAuthKey)
{
    ...
    else
    {
        ...
        {
            auth.append("&s=");
            auth.append(Base64Str<NODEHANDLE>(mPreviewSet->first.publicId()));
        }
    }
...
{
    if (!inSetPreviewMode()) std::cout << "[DEV] ::fetchSet but setPreviewMode not activated\n";
    ...
    auto clientUpdateOnCompletion =
        [completion, this](Error e, Set* s, map<handle, SetElement>* els)
        {
            ...
            if ((e == API_OK) && s && els)
            {
                auto& previewSet = mPreviewSet->first;
                previewSet = *s;
                auto& previewMapElements = mPreviewSet->second;
...
error MegaClient::startSetPreview(const char* publicSetLink)
{
    ...
    if ((e = parsepubliclink(publicSetLink, publicSetId, publicSetKey.data(), false)) == API_OK)
    {
        ...
        mPreviewSet = make_unique<pair<Set, map<handle, SetElement>>>(Set{},map<handle, SetElement>());
        mPreviewSet->first.setPublicId(publicSetId);

        key.setkey(publicSetKey.data());
        // upon loginToFolder, don't load the existing (if any) cache, since it's generated by
        // a previous "session" and it could be outdated. Better to create a fresh one
        openStatusTable(false);
    }
