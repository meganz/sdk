    ...
    for (file_list::iterator it = files.begin(); it != files.end(); it++)
    {
        if ( (*it)->failed(e)
                || (e == API_ENOENT // putnodes returned -9, file-storage server unavailable
                    && type == PUT
                    && slot && slot->transferbuf.tempUrlVector().empty()
                    && failcount < 16) )
        {
            defer = true;
        }
...
{
    for (unsigned connectionNum = reqs.size(); connectionNum--; )
    {
...
DirectReadSlot::DirectReadSlot(DirectRead* cdr)
{
    ...
    assert(reqs.empty());
    for (unsigned i = dr->drbuf.tempUrlVector().size(); i--; )
    {
