...
MegaNode *MegaApiImpl::getSyncedNode(const LocalPath& path)
{
    ...
    for (sync_list::iterator it = client->syncs.begin(); (it != client->syncs.end()) && (node == NULL); it++)
    {
        ...
        {
            if (Node* cr = sync->cloudRoot())
            {
                node = MegaNodePrivate::fromNode(cr);
                break;
            }
        }

        if (LocalNode* localNode = sync->localnodebypath(NULL, path))
        {
            if (Node* n = client->nodeByHandle(localNode->syncedCloudNodeHandle))
            {
                node = MegaNodePrivate::fromNode(n);
            }
        }
    }
    sdkMutex.unlock();
...
void MegaApiImpl::sendPendingRequests()
{
    ...
    while(MegaRequestPrivate *request = requestQueue.pop())
    {
        ...
        case MegaRequest::TYPE_SHARE:
        {
            ...
            if (e == API_OK)
            {
                client->setshare(node, email, a, nullptr, nextTag, [this, request](Error e){
                    fireOnRequestFinish(request, make_unique<MegaErrorPrivate>(e));
                });
            }
            break;
        ...
        case MegaRequest::TYPE_EXPORT:
        {
            ...
            e = client->exportnode(node, !request->getAccess(), request->getNumber(), nextTag, [this, request](Error e, handle h, handle ph){
        ...
        case MegaRequest::TYPE_REMOVE_SYNC:
        {
            ...
            while(it != client->syncs.end())
            {
                ...
                if ((tag && tag == sync->tag) //find matching (tag or node handle)
                        || !sync->cloudRoot() || sync->cloudRoot()->nodehandle == nodehandle)
                {
