...
MegaApiImpl::~MegaApiImpl()
{
    ...
    fireOnRequestFinish(request, new MegaErrorPrivate(API_OK));
}
...
void MegaApiImpl::abortPendingActions(error preverror)
{
    ...
    for (auto request : requests)
    {
        ...
        fireOnRequestFinish(request, new MegaErrorPrivate(preverror));
    }
    requestMap.clear();
    ...
    // -- Transfers in progress --
    {
        ...
        while (!transferMap.empty())
        {
            ...
            fireOnTransferFinish(transfer, new MegaErrorPrivate(preverror), committer);
        }
        assert(transferMap.empty());
        transferMap.clear();
...
bool MegaApiImpl::pread_data(byte *buffer, m_off_t len, m_off_t, m_off_t speed, m_off_t meanSpeed, void* param)
{
    ...
    if (!fireOnTransferData(transfer) || end)
    {
        ...
        DBTableTransactionCommitter committer(client->tctable);
        fireOnTransferFinish(transfer, end ? new MegaErrorPrivate(API_OK) : new MegaErrorPrivate(API_EINCOMPLETE), committer);
        return end;
    }
    return true;
}
...
void MegaApiImpl::reportevent_result(error e)
{
    ...
    fireOnRequestFinish(request, new MegaErrorPrivate(e));
}
...
void MegaApiImpl::queryrecoverylink_result(int type, const char *email, const char *ip, time_t, handle uh, const vector<string> *)
{
    ...
    {
        fireOnRequestFinish(request, new MegaErrorPrivate());
        return;
    }
    ...
    else if (reqType == MegaRequest::TYPE_CONFIRM_CHANGE_EMAIL_LINK)
    {
        if (type != CHANGE_EMAIL)
        {
            ...
            fireOnRequestFinish(request, new MegaErrorPrivate(API_EARGS));
            return;
        }
        ...
        if ((code = strstr(request->getLink(), "#verify")))
        {
            ...
            {
                fireOnRequestFinish(request, new MegaErrorPrivate(API_ENOENT));
                return;
            }
            ...
            {
                LOG_warn << "Version of account not supported";
                fireOnRequestFinish(request, new MegaErrorPrivate(API_EINTERNAL));
            }
            client->reqtag = creqtag;
        }
        else
...
void MegaApiImpl::getprivatekey_result(error e, const byte *privk, const size_t len_privk)
{
    ...
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(API_EKEY));
        return;
    }
...
void MegaApiImpl::getwelcomepdf_result(handle ph, string *key, error e)
{
    ...
    {
        return fireOnRequestFinish(request, new MegaErrorPrivate(API_OK));    // if import fails, notify account was successfuly created anyway
    }
}
...
void MegaApiImpl::syncupdate_state(Sync *sync, syncstate_t newstate)
{
    ...
    if (newstate == SYNC_FAILED)
    {
        ...
        fireOnRequestFinish(request, new MegaErrorPrivate(sync->errorcode));
    }
...
void MegaApiImpl::putnodes_result(error e, targettype_t t, NewNode* nn)
{
    ...
    if (request->getType() != MegaRequest::TYPE_MOVE)
    {
        ...
        {
            fireOnRequestFinish(request, new MegaErrorPrivate(API_OK));    // even if import fails, notify account was successfuly created anyway
            return;
        }
        fireOnRequestFinish(request, new MegaErrorPrivate(e));
    }
...
void MegaApiImpl::fa_complete(handle, fatype, const char* data, uint32_t len)
{
    ...
    while(tag)
    {
        ...
        fireOnRequestFinish(request, new MegaErrorPrivate(success ? API_OK : API_EWRITE));
    }
}
...
int MegaApiImpl::fa_failed(handle, fatype, int retries, error e)
{
    ...
    while(tag)
    {
        ...
        {
            fireOnRequestTemporaryError(request, new MegaErrorPrivate(e));
        }
    }
...
void MegaApiImpl::checkout_result(const char *errortype, error e)
{
    ...
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(e - 100));
        return;
    }

    fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::PAYMENT_EGENERIC));
    return;
}
...
void MegaApiImpl::notify_retry(dstime dsdelta, retryreason_t reason)
{
    ...
    if (dsdelta && requestMap.size() == 1)
    {
        ...
        fireOnRequestTemporaryError(request, new MegaErrorPrivate(API_EAGAIN, reason));
    }
}
    ...
    if (request->getType() == MegaRequest::TYPE_LOGIN)
    {
        ...
        if (version == 1)
        {
            ...
            else
            {
                ...
                {
                    fireOnRequestFinish(request, new MegaErrorPrivate(err));
                    return;
                }
...
void MegaApiImpl::login_result(error result)
{
    ...
    fireOnRequestFinish(request, new MegaErrorPrivate(result));
}
...
void MegaApiImpl::pubkey_result(User *u)
{
    ...
    if(!u->pubk.isvalid())
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(API_EACCESS));
        return;
    }
...
void MegaApiImpl::exportnode_result(handle h, handle ph)
{
    ...
    if ((n = client->nodebyhandle(h)))
    {
        ...
        {
            fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::API_EKEY));
            return;
        }

        string link = client->getPublicLink(client->mNewLinkFormat, n->type, ph, key);
        request->setLink(link.c_str());
        fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::API_OK));
    }
    else
    {
        request->setNodeHandle(UNDEF);
        fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::API_ENOENT));
    }
}
...
void MegaApiImpl::openfilelink_result(handle ph, const byte* key, m_off_t size, string* a, string* fa, int)
{
    ...
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::API_EACCESS));
        return;
    }
    ...
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::API_EINCOMPLETE));
        return;
    }
            ...
            || (request->getType() == MegaRequest::TYPE_CREATE_ACCOUNT))
    {
        ...
        if (!target)
        {
            fireOnRequestFinish(request, new MegaErrorPrivate(MegaError::API_EARGS));
            return;
        }
    ...
    switch (type)
    {
        case MegaApi::USER_ATTR_AVATAR:
            if (len)
            {
                ...
                if (!success)
                {
                    fireOnRequestFinish(request, new MegaErrorPrivate(API_EWRITE));
                    return;
                }
            }
...
void MegaApiImpl::whyamiblocked_result(int code)
{
    ...
    if (code <= 0)
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(code));
    }
    else    // code > 0
    {
...
void MegaApiImpl::multifactorauthcheck_result(int enabled)
{
    ...
    if (enabled < 0)
    {
        fireOnRequestFinish(request, new MegaErrorPrivate(enabled));
        return;
    }
...
void MegaApiImpl::processTransferComplete(Transfer *tr, MegaTransferPrivate *transfer)
{
    ...
    if (tr->type == GET)
    {
        ...
        transfer->setState(MegaTransfer::STATE_COMPLETED);
        DBTableTransactionCommitter committer(client->tctable);
        fireOnTransferFinish(transfer, new MegaErrorPrivate(API_OK), committer);
    }
    else
    {
        totalUploadedBytes += deltaSize;
                    ...
                    *static_cast<FileFingerprint*>(f) = fp;  // deliberate slicing - startxfer would re-fingerprint if we don't supply this info
                    ...
                    if (!started)
                    {
                        ...
                        if (!f->isvalid)
                        {
                            ...
                            transfer->setUpdateTime(Waiter::ds);
                            transfer->setState(MegaTransfer::STATE_FAILED);
                            fireOnTransferFinish(transfer, new MegaErrorPrivate(API_EREAD), committer);
                        }
                        else
                        {
                            ...
                            fireOnTransferFinish(transfer, new MegaErrorPrivate(API_EEXIST), committer);
                        }
                    }
                    currentTransfer = NULL;
                }
            ...
            case MegaTransfer::TYPE_DOWNLOAD:
            {
                ...
                if (!transfer->isStreamingTransfer())
                {
                    ...
                    if (!ok)
                    {
                        ...
                        if (overquotaDelay)
                        {
                            fireOnTransferTemporaryError(transfer, new MegaErrorPrivate(API_EOVERQUOTA, overquotaDelay));
                        }
        ...
        case MegaRequest::TYPE_CANCEL_ATTR_FILE:
        {
            ...
            if (!e)
            {
                ...
                while(it != requestMap.end())
                {
                    ...
                    {
                        fireOnRequestFinish(r, new MegaErrorPrivate(API_EINCOMPLETE));
                    }
                }
                fireOnRequestFinish(request, new MegaErrorPrivate(e));
            }
            ...
            if (found)
            {
                if (!flag)
                {
                    ...
                    if (e)
                    {
                        ...
                        fireOnRequestFinish(requestabort, new MegaErrorPrivate(API_OK));
                    }
                    else
                    {
                        request->setFlag(true);
...
void MegaFolderUploadController::start(MegaNode*)
{
    ...
    if(!parent)
    {
        transfer->setState(MegaTransfer::STATE_FAILED);
        DBTableTransactionCommitter committer(client->tctable);
        megaApi->fireOnTransferFinish(transfer, new MegaErrorPrivate(API_EARGS), committer);
    }
    else
    {
...
void MegaFolderUploadController::cancel()
{
    ...
    while (!subTransfers.empty())
    {
        ...
        DBTableTransactionCommitter committer(client->tctable);
        megaApi->fireOnTransferFinish(subTransfer, new MegaErrorPrivate(API_EINCOMPLETE), committer);
    }
}
...
void MegaBackupController::start(bool skip)
{
    ...
    if(!parent)
    {
        ...
        megaApi->fireOnBackupFinish(this, new MegaErrorPrivate(API_ENOENT));
    ...
    else
    {
        ...
        else
        {
            ...
            megaApi->fireOnBackupFinish(this, new MegaErrorPrivate(API_EEXIST));
            state = BACKUP_ACTIVE;
...
bool MegaBackupController::checkCompletion()
{
    if(!recursive && !pendingFolders.size() && !pendingTransfers && !pendingTags)
    {
        ...
        state = BACKUP_ACTIVE;
        megaApi->fireOnBackupFinish(this, new MegaErrorPrivate(e));
        megaApi->fireOnBackupStateChanged(this);
...
void MegaBackupController::abortCurrent()
{
    ...
    {
        megaApi->fireOnBackupFinish(this, new MegaErrorPrivate(API_EINCOMPLETE));
    }
...
void MegaFolderDownloadController::start(MegaNode *node)
{
    ...
    if (!node)
    {
        ...
        if (!node)
        {
            ...
            DBTableTransactionCommitter committer(client->tctable);
            megaApi->fireOnTransferFinish(transfer, new MegaErrorPrivate(API_ENOENT), committer);
            return;
        }
        deleteNode = true;
    }
