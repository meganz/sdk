...
: MegaNode()
{
    ...
    this->tag = static_cast<MegaNodePrivate*>(node)->tag; // deprecated; will be removed in future (this syntax just to avoid the warning)
...
bool MegaApiImpl::isInRootnode(MegaNode *node, int index)
{
    ...
    ret = (rootnode && client->rootnodes[index].eq(rootnode->getHandle()));
    delete rootnode;
...
void MegaApiImpl::authorizeMegaNodePrivate(MegaNodePrivate *node)
{
    ...
    if (node->getType() == MegaNode::TYPE_FILE)
    {
        ...
        {
            h = MegaApiImpl::handleToBase64(client->rootnodes[0].as8byte());
...
MegaNodeList* MegaApiImpl::search(MegaNode *n, const char* searchString, MegaCancelToken *cancelToken, bool recursive, int order, int type, int target)
{
    ...
    else
    {
        ...
        if (target == MegaApi::SEARCH_TARGET_OUTSHARE)
        {
            ...
            ::mega::unique_ptr<MegaShareList> shares(getOutShares(MegaApi::ORDER_NONE));
            for (int i = 0; i < shares->size() && !(cancelToken && cancelToken->isCancelled()); i++)
            {
...
void MegaApiImpl::fetchnodes_result(const Error &e)
{
    ...
    if (!client->restag)
    {
        ...
        if (e == API_OK)
        {
            assert(!client->rootnodes[0].isUndef());    // is folder link fetched properly?
        ...
        if (request->getType() == MegaRequest::TYPE_ADD_SYNC && e == API_OK)
        {
                                ...
                                [this, request](UnifiedSync *unifiedSync, const SyncError &syncError, error e)
            {
                ...
                if (unifiedSync)
                {
                    ...
                    auto sync = ::mega::make_unique<MegaSyncPrivate>(unifiedSync->mConfig, unifiedSync->mSync && unifiedSync->mSync->state() >= 0, client);
...
void MegaFolderDownloadController::downloadFolderNode(MegaNode *node, LocalPath& localpath, FileSystemType fsType)
{
    ...
    for (int i = 0; i < children->size(); i++)
    {
        ...
        ScopedLengthRestore restoreLen(localpath);
        localpath.appendWithSeparator(LocalPath::fromName(child->getName(), *client->fsaccess, fsType), true);
