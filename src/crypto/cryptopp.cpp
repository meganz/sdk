...
void SymmCipher::xorblock(const byte* src, byte* dst)
{
	...
	for (int i = BLOCKSIZE/sizeof(long); i--; ) ldst[i] ^= lsrc[i];
}
...
{
	if ((int)key[0].ByteCount()+2 > buflen) return 0;

	if (buf != plain) memcpy(buf,plain,plainlen);
...
void AsymmCipher::serializeintarray(Integer* t, int numints, string* d)
{
	...
	for (int i = numints; i--; ) size += t[i].ByteCount()+2;
