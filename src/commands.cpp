...
CommandSendEvent::CommandSendEvent(MegaClient *client, int type, const char *desc, bool addJourneyId, const char *viewId)
{
    ...
    if (addJourneyId)
    {
        if (client->journeyIdHasValue())
        {
            ...
            arg("ms", currentms);
            LOG_verbose << "[CommandSendEvent::CommandSendEvent] Attach journeyID = '" << client->getJourneyId() << "', ms = '" << currentms << "'";
        }
        else
        {
            LOG_warn << "[CommandSendEvent::CommandSendEvent] Add JourneyID flag is ON, but there is no JourneyID value set [add viewId: " << (viewId != nullptr) << "]";
        }
    }
    // Attach ViewID (generated by the SDK for the client, handled by the client)
    if (viewId)
    {
        LOG_verbose << "[CommandSendEvent::CommandSendEvent] Attach viewId = '" << viewId << "'";
        arg("v", viewId);
    }
