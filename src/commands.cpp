...
CommandChatCreate::CommandChatCreate(MegaClient *client, bool group, bool publicchat, const userpriv_vector *upl, const string_map *ukm, const char *title, bool meetingRoom, int chatOptions)
{
    ...
    mChatOptions = ChatOptions(static_cast<uint8_t>(chatOptions));
    ...
    if (meetingRoom)
    {
        ...
        // add chat options (just for meeting rooms)
        if (mChatOptions.speakRequest())     { arg("sr", 1); } // speak request
        if (mChatOptions.waitingRoom())      { arg("w",  1); } // waiting room
        if (mChatOptions.openInvite())       { arg("oi", 1); } // open invite
    }
            ...
            switch (client->json.getnameid())
            {
                ...
                case EOO:
                    if (chatid != UNDEF && shard != -1)
                    {
                        ...
                        chat->chatOptions = mChatOptions.value();
...
CommandSetChatOptions::CommandSetChatOptions(MegaClient* client, handle chatid, const string_map* options)
{
    this->client = client;
    mChatid = chatid;
    cmd("mco");
    ...
    {
        // iterate through string map searching for valid chat options
        string_map::const_iterator it = mChatOptions.find(std::to_string(i));
        if (it != mChatOptions.end())
        {
            // Entries with an invalid chat option, or with a non valid value (different from 0 or 1) will be ignored
            std::string optStr = ChatOptions::toApiParamStr(i); // returns expected field name by API in mco command
            std::string optVal = it->second.c_str();
            if (!optStr.empty() &&
                    !optVal.empty() &&
                    (!optVal.compare("0") || !optVal.compare("1")))
            {
                arg(optStr.c_str(), std::atoi(optVal.c_str()));
            }
            else
            {
                assert(false);
                LOG_err << "Invalid chat option or value provided "<< optStr << ": " << optVal;
            }
        }
    }
