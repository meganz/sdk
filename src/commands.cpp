...
CommandSendEvent::CommandSendEvent(MegaClient *client, int type, const char *desc, bool addJourneyId, const char *viewId)
{
    ...
    if (addJourneyId)
    {
        ...
        {
            LOG_warn << "[CommandSendEvent::CommandSendEvent] Add JourneyID flag is ON, but there is no JourneyID -or tracking flag is OFF- [add viewId: " << (viewId != nullptr) << "]";
        }
    }
