...
CommandSetChatOptions::CommandSetChatOptions(MegaClient* client, handle chatid, int chatOptions, bool add)
{
    this->client = client;
    mChatid = chatid;
    mAdd = add;
    ...
    // add chat options (just for meeting rooms)
    mChatOptions = ChatOptions(static_cast<uint8_t>(chatOptions));
    if (mChatOptions.speakRequest())     { arg("sr", add ? 1 : 0); } // speak request
    if (mChatOptions.waitingRoom())      { arg("w",  add ? 1 : 0); } // waiting room
    if (mChatOptions.openInvite())       { arg("oi", add ? 1 : 0); } // open invite
...
bool CommandSetChatOptions::procresult(Result r)
{
    if (r.wasError(API_OK))
    {
        ...
        chat->addOrUpdateChatOptions(mChatOptions.speakRequest() ? mAdd : -1,
                                     mChatOptions.waitingRoom()  ? mAdd : -1,
                                     mChatOptions.openInvite()   ? mAdd : -1);
