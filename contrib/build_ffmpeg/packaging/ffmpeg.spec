...
for i in `find target/include -type f | grep -v openssl `; do %{__install} -D -m 444 $i $RPM_BUILD_ROOT%{_includedir}/ffmpeg-mega/${i/target\/include\//}; done

(cd $RPM_BUILD_ROOT; for i in `find ./%{_libdir} -type f`; do echo $i | sed "s#^./#/#g"; done) >> %{_topdir}/ExtraFiles.list
echo /%{_includedir}/ffmpeg-mega  >> %{_topdir}/ExtraFiles.list
