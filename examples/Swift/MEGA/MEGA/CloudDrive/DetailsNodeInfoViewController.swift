...
class DetailsNodeInfoViewController: UIViewController, MEGADelegate, UIAlertViewDelegate {
    ...
    @IBAction func touchUpInsideDownload(_ sender: UIButton) {
        if node.type == MEGANodeType.file {
            ...
            if !fileExists {
                megaapi.startDownloadNode(node, localPath: documentFilePath, fileName: nil, appData: nil, startFirst: false, cancelToken: MEGACancelToken(), downloadOption: DownloadOptionSkipIfSameFingerprint, saveOption: SaveOptionNewWithN)
            }
        }
    }
