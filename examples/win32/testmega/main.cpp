...
void UploadVideos(const std::experimental::filesystem::path& path, MegaApi *megaApi, MegaNode* parentNode)
{
    if (std::experimental::filesystem::is_directory(path))
    {
        for (auto f = std::experimental::filesystem::directory_iterator(path); f != std::experimental::filesystem::directory_iterator(); ++f)
        {
            UploadVideos(f->path(), megaApi, parentNode);
        }
    }
    else
    {
        megaApi->startUpload(path.string().c_str(), parentNode, path.filename().string().c_str());
    }
}
...
int main()
{
    ...
    {
        std::experimental::filesystem::remove("C:\\tmp\\MegaSession.txt");
    }


    assert(megaApi->isLoggedIn());
    ...
    //UploadVideos("C:\\Users\\MATTW\\Desktop\\test_videos_standaloneinstaller\\", megaApi, rootNode.get());
