...
class MegaShell(cmd.Cmd, MegaListener):
    ...
    def do_ls(self, arg):
        ...
        if len(args) == 0:
            ...
            path = self._api.get_node_by_path(args[0], self.cwd)

        print('    .')
        #if self._api.get_parent_node(path) != None:
            #print('    ..')

        nodes = self._api.get_children(path)
    ...
    def do_cd(self, arg):
        ...
        if len(args) == 0:
            self.cwd = self._api.get_root_node()
            return

        node = self._api.get_node_by_path(args[0], self.cwd)
        if node == None:
    ...
    def do_mkdir(self, arg):
        ...
        if '/' in name or '\\' in name:
            ...
            if index1 > index2:
                ...
                index = index2
            path = name[:index + 1]
            base = self._api.get_node_by_path(path, self.cwd)
            name = name[index + 1:]
        ...
        check = self._api.get_node_by_path(name, base)
        if check != None:
    ...
    def do_mv(self, arg):
        ...
        src_node = self._api.get_node_by_path(args[0], self.cwd)
        if src_node == None:
            print('{}: No such file or directory'.format(args[0]))
            return

        name = args[1]
        dst_node = self._api.get_node_by_path(name, self.cwd)
