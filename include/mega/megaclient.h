...
public:
    ...
    void sc_procEoo(std::unique_lock<recursive_mutex>& nodeTreeIsChanging, const bool originalAC);
    ...
    bool sc_checkActionPacket(JSON& json, const Node* const lastAPDeletedNode);
    // check if it is ok to process the current action packet when there's no st
    bool sc_checkActionPacketWithoutSt(const nameid cmd, const Node* const lastAPDeletedNode);
    // enter json object to check the action packet, then restore json position
    bool sc_checkActionPacketPreservePos(JSON& json, const Node* const lastAPDeletedNode);
