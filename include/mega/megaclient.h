...
public:
    ...
    // fetch backups from BC, search bkpId among them, disable the backup, update sds attribute, move or delete its contents
    void removeBackupMD(handle bkpId, handle bkpDest, std::function<void(const Error&)> f);

    // fetch backups from BC
